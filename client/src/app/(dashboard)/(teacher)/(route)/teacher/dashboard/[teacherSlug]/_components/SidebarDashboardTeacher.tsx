...
import ButtonLogout from "@/app/(dashboard)/(student)/(route)/dashboard/[studentId]/profile/_components/ButtonLogout";
    ...
    <>
      ...
      <div className="fixed bottom-0 left-0 z-20 flex justify-around w-full p-2 bg-white border-t border-gray-300 shadow-md lg:hidden">
        {itemSidebar.map(({ Icon, content, name }, index) => (
          <p
            key={index}
            className={`flex flex-col items-center transition-all ease-in-out hover:text-primary-color/80 cursor-pointer ${
              content === contentView
                ? "text-primary-color"
                : "text-text-primary"
            }`}
        ...
        <p
          className={`flex flex-col cursor-pointer items-center transition-all ease-in-out hover:text-primary-color/80 ${
            contentView === "setting"
              ? "text-primary-color"
              : "text-text-primary"
          }`}
